...
function createArrow(direction, origin, length, color, name) {
    ...
    // Shaft
    const shaftGeometry = new THREE.CylinderGeometry(length / 70, length / 50,
        length - 1000 / 20, 16);
    const shaftMaterial = new THREE.MeshBasicMaterial({
        color
    });
    const shaft = new THREE.Mesh(shaftGeometry, shaftMaterial);
    shaft.position.copy(origin.clone()
        .add(normalizedDirection.clone()
            .multiplyScalar((length - 1000 / 20) / 2)));
    ...
    head.position.copy(origin.clone()
        .add(normalizedDirection.clone()
            .multiplyScalar(length - 500 / 20)));
...
export function createFrame(pos, length) {
    ...
    const xAxisArrow = createArrow(new THREE.Vector3(1, 0, 0), new THREE
        .Vector3(pos.x, pos.y, pos.z), length, 0xff0000, "xaxis"); // X-axis (red)
    const yAxisArrow = createArrow(new THREE.Vector3(0, 1, 0), new THREE
        .Vector3(pos.x, pos.y, pos.z), length, 0x00ff00, "yaxis"); // Y-axis (green)
    const zAxisArrow = createArrow(new THREE.Vector3(0, 0, 1), new THREE
        .Vector3(pos.x, pos.y, pos.z), length, 0x0000ff, "zaxis"); // Z-axis (blue)
...
// Floating Points
export function createFloatingPoint(radius = 50, widthSegments = 16,
    heightSegments =
    16, color = 0xff0000) {
    const pointGroup = new THREE.Group();
    const geometry = new THREE.SphereGeometry(radius, widthSegments,
        heightSegments);
